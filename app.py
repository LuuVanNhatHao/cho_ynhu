...
def load_data():
    ...
    try:
        if not os.path.exists('data.csv'):
            return False, "Data file not found. Please ensure data.csv is in the current directory"
        ...
        return True, f"Successfully loaded {len(df)} records with {len(df.columns)} columns"
